...
public class FlowLayout extends ViewGroup {
	...
	private int singleLineControlMaxThickness=0;
	private int sn=0;
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		for (int i = 0; i < count; i++) {
			...
			//gz
			if (singleLine&&posY>0&&sn==0) {
				singleLineControlMaxThickness=prevLinePosition;
				sn++;
			}
		}
		...
		if (orientation == HORIZONTAL) {
			...
			//gz
			if (singleLine&&singleLineControlMaxThickness>0) {
				this.setMeasuredDimension(
						resolveSize(controlMaxLength, widthMeasureSpec),
						resolveSize(singleLineControlMaxThickness, heightMeasureSpec));
				int ss=singleLineControlMaxThickness;
			}else {
				this.setMeasuredDimension(
						resolveSize(controlMaxLength, widthMeasureSpec),
